...
export default function DoctorTicketDetail() {
  ...
  const handleStatusChange = (newStatus) => {
    if (typeof window.TicketAPI === 'undefined' || !window.TicketAPI.updateTicketStatus) return;
  ...
  if (error || !ticket) {
    return (
      <>
        ...
        <Link href="/doctor/tickets" className="btn-primary">Back to Tickets</Link>
      </>
    );
  }
              ...
              <div className="status-dropdown" aria-expanded={statusOpen}>
                ...
                <ul className="status-dropdown-menu">
                  {STATUS_OPTIONS.map((opt) => (
                    <li
                      key={opt.value}
                      className={`status-dropdown-option ${opt.value === statusValue ? 'selected' : ''}`}
                      onClick={() => { setStatusValue(opt.value); setStatusOpen(false); handleStatusChange(opt.value); }}
                    >
